...
export default function ApartadoDetallePage() {
  ...
  const prendasAgrupadas = agruparPrendasApartado(grupo.items);
  const cantidadPendientes = grupo.items.filter(item => item.estado === "pendiente").length;

  function toggleGrupoPrendas(key: string) {
    setGruposExpandidos(actuales => {
  ...
  function renderAccionesPrenda(item: ItemGrupo) {
    if (item.estado !== "pendiente" || !isAdmin) return null;
    return (
    ...
    <div className="max-w-2xl mx-auto px-4 md:px-8 pt-6">
      ...
      <div className="card mb-4">
        ...
        <div className="space-y-3">
          {prendasAgrupadas.map(grupoPrendas => {
            const item = grupoPrendas.items[0];
            const expandido = gruposExpandidos.has(grupoPrendas.key);
            const observaciones = Array.from(new Set(
              grupoPrendas.items.map(unidad => unidad.observacion).filter(Boolean),
            ));
            return (
              <div key={grupoPrendas.key} className="border border-gray-100 rounded-xl p-3">
              <div className="flex items-start justify-between mb-2">
                <div className="flex-1">
                  ...
                  {observaciones.map(observacion => (
                    <p key={observacion} className="text-xs text-gray-400 italic mt-0.5">"{observacion}"</p>
                  ))}
                </div>
                <div className="text-right ml-3">
                  {grupoPrendas.cantidad > 1 && (
                    <p className="text-xs text-gray-500">
                      {grupoPrendas.cantidad} unidades × {formatCurrency(grupoPrendas.precioUnitario)}
                    </p>
                  )}
                  <p className="font-semibold text-sm">{formatCurrency(grupoPrendas.subtotal)}</p>
              ...
              <div className="flex items-center justify-between mt-2 pt-2 border-t border-gray-50">
                ...
                {grupoPrendas.cantidad === 1
                  ? renderAccionesPrenda(item)
                  : (
                    <button
                      onClick={() => toggleGrupoPrendas(grupoPrendas.key)}
                      className="flex items-center gap-1 text-xs text-brand-blue font-semibold hover:underline"
                      aria-expanded={expandido}
                    >
                      Gestionar {grupoPrendas.cantidad} unidades
                      <ChevronDown className={`w-3.5 h-3.5 transition-transform ${expandido ? "rotate-180" : ""}`} />
                    </button>
                  )}
              </div>

              {grupoPrendas.cantidad > 1 && expandido && (
                <div className="mt-3 pt-3 border-t border-gray-100 space-y-2">
                  {grupoPrendas.items.map((unidad, index) => (
                    <div key={unidad.id} className="flex items-center justify-between gap-3 rounded-lg bg-gray-50 px-3 py-2">
                      <div>
                        <p className="text-xs font-semibold text-gray-700">Unidad {index + 1}</p>
                        <p className="text-[11px] text-gray-400">{formatCurrency(unidad.precio)}</p>
                      </div>
                      {renderAccionesPrenda(unidad)}
                    </div>
                  ))}
                </div>
              )}
