...
- **Apartados use one row per unit.** All rows of the same pedido share a `grupo_id`. The `en_tienda` flag encodes whether the unit was already deducted from stock. Creating an apartado with `en_tienda=true` inserts a `salida/ajuste` immediately. "Marcar recibida" (`en_tienda: false → true`) checks if stock is available: if yes, deducts it and sets the flag; if no (prenda went direct from supplier to client), only sets the flag. Canceling an apartado with `en_tienda=true` returns the unit via `entrada/ajuste`.
